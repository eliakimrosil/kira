...
@contextmanager
def no_alsa_error():
    """Context manager to suppress ALSA and JACK error messages by redirecting stderr."""
    # Set ALSA error handler via ctypes
    try:
    ...
    # Redirect stderr to /dev/null to silence JACK and other noisy C libraries
...
5. Music Playback: Expert in 'mpv' for playing local music from '~/GoogleDrive/Music'. 
   - ALWAYS play in background: 'nohup mpv --input-ipc-server=/tmp/mpv-socket --no-video [target] > /dev/null 2>&1 &'
   - NEXT/PREV: Use 'kira-mpv-ctrl next' or 'kira-mpv-ctrl prev'.
   - PAUSE/PLAY: Use 'kira-mpv-ctrl pause' or 'kira-mpv-ctrl play'.
   - STOP: Use 'pkill mpv'.
6. Bluetooth Management: Expert in 'bluetoothctl' for managing Bluetooth devices.
7. System Control: Expert in power management and general maintenance.
                ...
                if message.tool_call:
                    ...
                    for call in message.tool_call.function_calls:
                        if call.name == "run_command":
                            ...
                            console.print(f"[bold yellow][Kira Action][/bold yellow] Executing: {cmd}")
                            ...
                            if res['code'] == 0:
                                console.print(f"[bold green][System][/bold green] Success")
                            else:
                                console.print(f"[bold red][System][/bold red] Failed (Code: {res['code']})")
                                if res['stderr']:
